...
export class PutUserComponent implements OnInit {
  ...
  getUserById() {
    return this.personService.getUserById(this.idUser).subscribe((res) => {
      this.dataUser = res;
      console.log(this.dataUser)
      this.person.names = this.dataUser.data.names;
      this.person.last_names = this.dataUser.data.last_names;
      this.person.phone = this.dataUser.data.phone;
      this.person.level_academy = this.dataUser.data.level_academy;
      this.person.rol = this.dataUser.data.rol;
      this.person.password = this.dataUser.data.password;
    });
  }
  ...
  putUser() {
    if (
      (this.person.names && this.person.last_names && this.person.phone &&
      this.person.level_academy && this.person.rol &&  this.person.password )
    ) {
      let dataPerson = {
        person: this.person,
      };
